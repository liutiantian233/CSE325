...
class Simulation {
    ...
    union _type {
        ...
        void print8() {
            for (int i = 3; i >= 0; --i)
                cout << hex << setw(2) << setfill('0') << (int) I_8[i] << " ";
        }
    };
    ...
    public:
        Cache_Page() {
            static_assert(sizeof(uint32_t) == sizeof(uint8_t) * 4, "wrong!!!");
            _valid = false;
            _modify = false;
            _tag_bits = 0;
            _data.resize(4);
        ...
        void print(const uint8_t offset) {
            _data[offset / 4].print8();
        }
        ...
        uint32_t _Get_Data(const uint8_t offset) const {
...
public:
    ...
    /*---------------------------------------------------------------------------
    Name: Write_Mem function
    Purpose: For Mem to write
    Return: no return
    ---------------------------------------------------------------------------*/
    void Write_Mem(uint32_t address, vector<_type> &data) {
        for (unsigned int i = 0; i < Cache_Data_Length; ++i) {

            _Storage[i + address] = data[i / 4].I_8[3 - i % 4];
//            cout << i + address << "   " << (int) _Storage[i + address] << endl;
    ...
    /*---------------------------------------------------------------------------
    Name: Read_Mem function
    Purpose: For Mem to read
    Return: no return
    ---------------------------------------------------------------------------*/
    void Read_Mem(uint32_t address, vector<_type> &data) {
        for (unsigned int i = 0; i < Cache_Data_Length; ++i)
    ...
    /*---------------------------------------------------------------------------
    Name: Print_Mem function
    Purpose: For Mem to print
    Return: no return
    ---------------------------------------------------------------------------*/
    void Print_Mem() {
        const int _start = 0x20000;
        const int _length = 128;
//        const int _start = 0x20ac0;
//        const int _length = 512;
        cout << "Memory shows from 0x20000 to 0x2007f " << endl;
        for (int i = _start; i < _start + _length; i += Cache_Data_Length) {
            cout << hex << setw(5) << setfill('0') << i << "  ";
            for (unsigned int j = 0; j < Cache_Data_Length; ++j) {
                cout << hex << setw(2) << setfill('0') << (int) _Storage[i + j] << " ";
            }
            cout << endl;
        }
        cout << "End." << endl;
...
//                    cout << endl << " RRRTTTT"
//                         << (_Lines[index]._Get_Tag() * MAX_Cache_Length + index) * Cache_Data_Length << endl;
                    Write_Mem((uint32_t) (_Lines[index]._Get_Tag() * MAX_Cache_Length + index) * Cache_Data_Length,
                              data);      //write back
...
//                         << (_Lines[index]._Get_Tag() * MAX_Cache_Length + index) * Cache_Data_Length << endl;
                    ...
                    Write_Mem((uint32_t) (_Lines[index]._Get_Tag() * MAX_Cache_Length + index) * Cache_Data_Length, temp);      //write back
                }
            }
        }
